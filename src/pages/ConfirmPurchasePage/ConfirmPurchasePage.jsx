...
export default function ConfirmPurchasePage() {
    ...
    async function handleSubmit(evt) {
        try {
            ...
            const body = {
                deliveryName: evt.target.name.value,
                deliveryContactNo: evt.target.contactNo.value,
                deliveryAddress: evt.target.address.value,
                purchaseInfo: purchaseToken
            }
            const acceptedReceipt = await sendRequest('/api/receipts/', 'POST', body)
            setPageState('idle')
            setBoughtReceipt(acceptedReceipt)
        } catch (err) {
    ...
    return (pageState === 'idle') && verifiedPurchase ?
            ...
            <div className="w-full grid grid-cols-[3fr_1fr]">
                <div className="flex flex-col items-start [&>*]:my-4 w-full">
                    ...
                    <p className="text-left">
                        ...
                        <p>Delivery of physical tickets will usually take 3 to 5 working days, but logistics delays may result in delivery times up to 3 weeks.</p>
                    </p>
                    <form id="deliveryDetails" onSubmit={handleSubmit}>
                        <fieldset className="grid grid-cols-2 text-left [&>*]:my-2 [&>input]:border-2 [&>input]:border-solid [&>input]:border-stone-500 [&>input]:rounded-lg items-start">
                            <label>Name:</label>
                            <input type="text" name="name" required defaultValue={userDetails.name} />

                            <label>Contact No:</label>
                            <input type="text" name="contactNo" required defaultValue={userDetails.contactNo} />

                            <label>Address:</label>
                            <input type="text" name="address" required defaultValue={userDetails.address} />
        ...
        <div className="[&>*]:my-4">
            <h1>{pageState}</h1>
            {pageState === "Something went wrong!" ? <button className="w-fit" onClick={() => window.location.reload()}>Try Again</button> : null}
        </div>
